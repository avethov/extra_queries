      ...
      base.class_eval do
        ...
        scope :eq_sidebar_queries, lambda { |project|
          self.visible
              ...
              .where(project ? ["NOT EXISTS(SELECT 1 FROM eq_queries_hide_in_projects eq_hp WHERE eq_hp.query_id = #{IssueQuery.table_name}.id AND (eq_hp.project_id = ? OR (#{IssueQuery.table_name}.sub_project = ? AND eq_hp.project_id IN (#{(project.ancestors.map(&:id) + [0]).join(', ')}) )))", project.id, true ]: '')
              .where(project ? ["NOT EXISTS(SELECT 1 FROM eq_queries_show_in_projects eq_sp WHERE eq_sp.query_id = #{IssueQuery.table_name}.id) OR EXISTS(SELECT 1 FROM eq_queries_show_in_projects eq_sp WHERE eq_sp.query_id = #{IssueQuery.table_name}.id AND (eq_sp.project_id = ? OR ( #{IssueQuery.table_name}.sub_project = ? AND eq_sp.project_id IN (#{(project.ancestors.map(&:id) + [0]).join(', ')}) )))", project.id, true] : '')
        }
        scope :eq_pinned, -> {
